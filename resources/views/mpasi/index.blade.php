...
@section('content')
    ...
    <script>
        ...
        function renderAdminPesananPerOutlet() { const selectedOutlet = document.getElementById('adm-pesanan-outlet-filter')?.value || 'ALL'; const todayStr = getTodayDateString(); const todayOrders = state.preOrders.filter(p => p.date === todayStr); const cardsEl = document.getElementById('adm-pesanan-outlet-cards'); if (cardsEl) { cardsEl.innerHTML = state.outlets.map(outletName => { const ordersInOutlet = todayOrders.filter(p => p.outlet === outletName); const totalOrders = ordersInOutlet.length; const belumDiambil = ordersInOutlet.filter(p => !p.isTaken && p.cancelStatus !== 'approved').length; const dibatalkan = ordersInOutlet.filter(p => p.cancelStatus === 'approved').length; return `<div class="col-md-4"><div class="card-custom p-3 h-100 border-start border-4 border-primary"><div class="fw-bold text-brand-purple fs-7 mb-2"><i class="fa-solid fa-shop me-1"></i> ${outletName}</div><div class="d-flex justify-content-between fs-8 mb-1"><span class="text-muted">Total Pesanan Hari Ini</span><span class="fw-bold">${totalOrders}</span></div><div class="d-flex justify-content-between fs-8 mb-1"><span class="text-muted">Untuk Diambil Besok</span><span class="fw-bold text-warning">${belumDiambil}</span></div><div class="d-flex justify-content-between fs-8"><span class="text-muted">Dibatalkan</span><span class="fw-bold text-danger">${dibatalkan}</span></div></div></div>`; }).join(''); } const tbody = document.getElementById('adm-pesanan-tbody'); const filteredOrders = selectedOutlet === 'ALL' ? todayOrders : todayOrders.filter(p => p.outlet === selectedOutlet); if (tbody) { tbody.innerHTML = filteredOrders.length > 0 ? filteredOrders.map(p => `<tr class="${p.isTaken ? 'bg-light opacity-75' : ''} ${p.cancelStatus === 'approved' ? 'table-danger' : ''}"><td class="fw-bold ${p.isTaken || p.cancelStatus === 'approved' ? 'text-decoration-line-through text-muted' : 'text-dark'}">${p.id} - ${p.customerName}</td><td><span class="badge bg-purple-light text-brand-purple border border-purple-200 fs-8">${p.outlet}</span></td><td><a href="https://wa.me/${p.wa}" target="_blank" class="text-success text-decoration-none fw-bold"><i class="fa-brands fa-whatsapp me-1"></i> ${p.wa}</a></td><td class="fs-8">${p.items}</td><td><span class="badge ${p.isPaid ? 'bg-success' : 'bg-danger'} fs-8">${p.isPaid ? 'Lunas ✅' : 'Belum Bayar (COD)'}</span></td><td><span class="badge ${p.isTaken ? 'bg-success' : 'bg-warning text-dark'} fs-8">${p.isTaken ? 'Sudah Diambil ✅' : 'Menunggu Ambil'}</span></td><td>${cancelInfoBadge(p)}</td></tr>`).join('') : `<tr><td colspan="7" class="text-center text-muted fs-8 fst-italic py-3">Belum ada pesanan masuk hari ini untuk diambil besok.</td></tr>`; } renderOrdersMenuSummary(filteredOrders, 'adm-pesanan-summary-content', 'adm-pesanan-total-badge', selectedOutlet !== 'ALL' ? selectedOutlet : 'Semua Outlet'); }
