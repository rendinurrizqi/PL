...
@section('content')
    ...
    <script>
        ...
        function purgeOldPreOrders() { const todayStr = getTodayDateString(); let changed = false; state.preOrders = (state.preOrders || []).filter(order => { if (!order.date) { order.date = todayStr; return true; } if (order.date !== todayStr) { changed = true; return false; } return true; }); if (changed) savePreOrdersToStorage(); }
