...
@section('content')
    ...
    <script>
        ...
        function renderCustomerProfilePage() { const container = document.getElementById('akun-page-content'); if (!container) return; if (!state.currentUser) { switchCustView('login'); return; } const member = state.currentUser; const oldIdentifier = member.identifier || member.wa; const outletsOptionsHtml = state.outlets.map(o => `<option value="${escAttr(o)}" ${(member.favoriteOutlet === o) ? 'selected' : ''}>${o}</option>`).join(''); container.innerHTML = `<div class="row g-4 max-w-1000 mx-auto"><div class="col-md-5"><div class="card-custom p-4 bg-purple-light border border-purple-200 h-100 d-flex flex-column justify-content-between"><div><div class="text-center mb-3"><div class="bg-brand-purple text-white d-inline-flex p-3 rounded-circle mb-2 fs-2 shadow-sm"><i class="fa-solid fa-circle-user"></i></div><h5 class="fw-bold text-brand-purple mb-0">${member.name}</h5><span class="badge bg-brand-yellow text-dark fs-8 fw-bold mt-1 px-3 py-1 rounded-pill"><i class="fa-solid fa-coins me-1"></i> ${member.points} Poin Belanja</span></div><hr class="border-purple-200"><div class="fs-7 text-dark space-y-2"><div class="d-flex justify-content-between py-1 border-bottom"><span class="text-muted">No. WhatsApp:</span><span class="fw-bold text-dark">${member.wa}</span></div><div class="d-flex justify-content-between py-1 border-bottom"><span class="text-muted">Outlet Favorit:</span><span class="fw-bold text-brand-purple">${member.favoriteOutlet || 'Belum Diatur'}</span></div><div class="d-flex justify-content-between py-1"><span class="text-muted">Status Akun:</span><span class="badge bg-success fs-8">Member Aktif</span></div></div></div><div class="mt-4 pt-2 border-top"><button class="btn btn-outline-danger w-100 fw-bold py-2 rounded-pill fs-7" onclick="logoutCustomer()"><i class="fa-solid fa-right-from-bracket me-1"></i> Keluar dari Akun</button></div></div></div><div class="col-md-7"><div class="card-custom p-4 shadow-sm"><h5 class="fw-bold text-brand-purple mb-1"><i class="fa-solid fa-user-pen me-2 text-warning"></i> Edit Profil Saya</h5><p class="text-muted fs-7 mb-4">Perbarui nama pemesan, nomor WhatsApp, atau outlet favorit langganan Bunda.</p><form id="profile-edit-form" onsubmit="handleSaveCustomerProfile(event)"><div class="mb-3"><label class="form-label fs-7 fw-bold text-dark">Nama Lengkap Bunda / Pemesan <span class="text-danger">*</span></label><input type="text" id="prof-name" class="form-control form-control-lg fs-7 fw-bold" placeholder="Contoh: Bunda Siti Rahmawati" value="${escAttr(member.name || '')}" required></div><div class="mb-3"><label class="form-label fs-7 fw-bold text-dark">Nomor WhatsApp Aktif <span class="text-danger">*</span></label><input type="tel" id="prof-wa" class="form-control form-control-lg fs-7 fw-bold" placeholder="Contoh: 081298765432" value="${escAttr(member.wa || oldIdentifier || '')}" required></div><div class="mb-4"><label class="form-label fs-7 fw-bold text-dark">Outlet Langganan Favorit</label><select id="prof-outlet" class="form-select form-select-lg fs-7"><option value="">-- Pilih Outlet Favorit --</option>${outletsOptionsHtml}</select><div class="form-text fs-8">Outlet ini akan otomatis terpilih saat Bunda melakukan transaksi checkout.</div></div><button type="submit" class="btn btn-brand-purple btn-lg w-100 py-3 fw-bold fs-6 shadow-sm"><i class="fa-solid fa-floppy-disk me-2"></i> SIMPAN PERUBAHAN PROFIL</button></form></div></div></div>`; }
