...
@section('content')
    ...
    <script>
        ...
        function renderHomeProducts() { const grid = document.getElementById('home-products-grid'); if (!grid) return; const todayProds = getTodayProducts(); const days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu']; const todayName = days[new Date().getDay()]; if (todayProds.length === 0) { grid.innerHTML = `<div class="col-12 text-center py-4 bg-purple-light rounded-3 text-muted"><i class="fa-solid fa-utensils fs-3 mb-2 text-brand-purple d-block"></i><h6 class="fw-bold text-dark">Belum Ada Menu MPASI untuk Hari ${todayName}</h6><p class="fs-8 mb-0">Menu rotasi harian belum diisi oleh Admin/Owner.</p></div>`; return; } grid.innerHTML = todayProds.slice(0, 3).map(p => ` <div class="col-md-4"><div class="card-custom p-3 h-100 d-flex flex-column justify-content-between"><div>${productImageHtml(p, 140)}<span class="badge bg-warning text-dark fs-8 fw-bold mb-1">${p.age}</span><h6 class="fw-bold mb-1 text-dark">${p.name}</h6><div class="text-muted fs-8 mb-2">${p.ingredients}</div></div><div><div class="fw-bold text-brand-purple fs-6 mb-2">Rp ${p.price.toLocaleString('id-ID')}</div><button class="btn btn-brand-yellow btn-sm w-100 fw-bold text-dark" onclick="addToCart('${p.id}')"><i class="fa-solid fa-cart-plus me-1"></i> + Tambah Ke Keranjang</button></div></div></div>`).join(''); }
        function renderCatalogProducts() { const grid = document.getElementById('full-products-grid'); if (!grid) return; const todayProds = getTodayProducts(); const days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu']; const todayName = days[new Date().getDay()]; if (todayProds.length === 0) { grid.innerHTML = `<div class="col-12 text-center py-5 bg-purple-light rounded-3 text-muted"><i class="fa-solid fa-calendar-xmark fs-1 mb-3 text-brand-purple d-block"></i><h5 class="fw-bold text-dark">Belum Ada Menu MPASI untuk Hari ${todayName}</h5><p class="fs-7 mb-0">Owner / Admin belum menambahkan varian menu rotasi harian untuk hari ${todayName}.</p></div>`; return; } grid.innerHTML = todayProds.map(p => ` <div class="col-md-4"><div class="card-custom p-3 h-100 d-flex flex-column justify-content-between"><div>${productImageHtml(p, 140)}<span class="badge bg-warning text-dark fs-8 fw-bold mb-1">${p.age}</span><h6 class="fw-bold mb-1 text-dark">${p.name}</h6><div class="text-muted fs-8 mb-2">${p.ingredients}</div></div><div><div class="fw-bold text-brand-purple fs-6 mb-2">Rp ${p.price.toLocaleString('id-ID')}</div><button class="btn btn-brand-yellow btn-sm w-100 fw-bold text-dark" onclick="addToCart('${p.id}')"><i class="fa-solid fa-cart-plus me-1"></i> + Tambah Ke Keranjang</button></div></div></div>`).join(''); }
