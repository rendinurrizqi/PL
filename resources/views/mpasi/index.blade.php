...
@section('content')
    ...
    <script>
        ...
        function decideCancelOrder(orderId, decision) { const order = state.preOrders.find(o => o.id == orderId); if (!order) return; const isApprove = decision === 'approved'; Swal.fire({ icon: isApprove ? 'warning' : 'question', title: isApprove ? 'Setujui Pembatalan Pesanan?' : 'Tolak Permintaan Pembatalan?', html: `Pesanan <b>${order.id}</b> a.n <b>${order.customerName}</b>${order.cancelReason && order.cancelReason !== '-' ? `<br><span class="fs-8 text-muted">Alasan: ${order.cancelReason}</span>` : ''}`, showCancelButton: true, confirmButtonText: isApprove ? 'Ya, Setujui Pembatalan' : 'Ya, Tolak Pembatalan', cancelButtonText: 'Batal', confirmButtonColor: isApprove ? '#dc3545' : '#8C52FF' }).then(res => { if (res.isConfirmed) { order.cancelStatus = decision; if (isApprove && order.pointsAwarded && order.memberIdentifier && state.members[order.memberIdentifier]) { const member = state.members[order.memberIdentifier]; member.points = Math.max(0, member.points - order.pointsAwarded); member.pointsHistory.unshift({ type: 'adjust', label: `Poin ditarik - pesanan ${order.id} dibatalkan`, points: -order.pointsAwarded, date: new Date().toLocaleString('id-ID') }); order.pointsAwarded = 0; } savePreOrdersToStorage(); renderAllUI(); Swal.fire({ icon: 'success', title: isApprove ? 'Pembatalan Disetujui' : 'Permintaan Ditolak', text: isApprove ? `Pesanan ${order.id} resmi dibatalkan.` : `Pesanan ${order.id} tetap diproses seperti biasa.`, timer: 1500, showConfirmButton: false }); } }); }
        ...
        function submitAllKasirLeftovers() { renderAdminOutletReports(); renderOwnerOutletReports(); Swal.fire({ icon: 'success', title: 'Rekap Laporan Dikirim!', text: 'Laporan rekapan penjualan, sisa produk, dan keuntungan untuk ' + state.kasirActiveOutlet + ' telah diteruskan ke Admin & Owner secara real-time!', confirmButtonColor: '#8C52FF' }); }
        ...
        function printDapurMasakReport(filterSelectId) {
                    ...
                    if (totalPorsiMasak > 0 || dayFilter !== 'ALL') {
                        ...
                        dayRows += `
                            <tr>
                                ...
                                <td style="padding: 6px 8px; border: 1px solid #ddd; text-align: center; font-weight: bold; color: #8C52FF;">${totalPorsiMasak} Cup</td>
                            </tr>
                        `;
                    }
                });
                ...
                if (dayRows) {
                    ...
                    rowsHtml += `
                        <tr style="background-color: #f3e5f5; font-weight: bold;">
                            <td colspan="5" style="padding: 6px 8px; border: 1px solid #ddd; color: #8C52FF;">
            ...
            const summaryRowHtml = totalItemCount > 0 ? `
                <tr style="background-color: #f8f9fa; font-weight: bold;">
                    ...
                    <td style="padding: 8px; border: 1px solid #ddd; text-align: center; color: #8C52FF; font-size: 14px;">${grandTotalAll} Cup</td>
                </tr>
            ...
            const printHtml = `
                ...
                <head>
                    ...
                    <style>
                        ...
                        th { background-color: #8C52FF; color: white; padding: 8px; border: 1px solid #8C52FF; text-align: left; }
                    </style>
                </head>
                <body onload="window.print(); setTimeout(() => window.close(), 600);">
