...
@section('content')
        ...
        <div id="role-portal-pelanggan" class="role-portal-page">
            <nav class="navbar navbar-expand-lg navbar-custom px-3">
                <div class="container-fluid max-w-1600">
                    ...
                    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#custNavContent">
                        <span class="navbar-toggler-icon"></span>
                    </button>

                    <div class="collapse navbar-collapse" id="custNavContent">
                ...
                <div class="flex-grow-1 p-4 overflow-auto">
                    ...
                    <div id="kasir-tab-pos" class="kasir-tab-content" style="display:none;">
                        ...
                        <div class="row g-3">
                            <div class="col-md-7">
                                <div class="card-custom p-3">
                                    <h6 class="fw-bold mb-3">Pilih Produk MPASI Ready Stock</h6>
    ...
    <script>
        let state = {
            ...
            dailyMenu: [
                { day: 'Senin', productIds: ['PRD-1', 'PRD-2', 'PRD-4'] },
                { day: 'Selasa', productIds: ['PRD-2', 'PRD-3', 'PRD-4'] },
                { day: 'Rabu', productIds: ['PRD-1', 'PRD-3'] },
                { day: 'Kamis', productIds: ['PRD-1', 'PRD-2', 'PRD-4'] },
                { day: 'Jumat', productIds: ['PRD-2', 'PRD-3'] },
                { day: 'Sabtu', productIds: ['PRD-1', 'PRD-4'] },
                { day: 'Minggu', productIds: ['PRD-1', 'PRD-2', 'PRD-3', 'PRD-4'] }
            ],
            preOrders: [],
            outletSalesRecords: {
        ...
        function productImageHtml(p, heightPx) { const h = heightPx || 110; if (p.image) { return `<img src="${p.image}" alt="${p.name}" class="rounded-3 mb-2 w-100" style="height:${h}px; object-fit:cover;">`; } return `<div class="${PRODUCT_IMG_PLACEHOLDER_CLASS}" style="height:${h}px;"><i class="fa-solid fa-bowl-food"></i></div>`; }
        ...
        function selectRolePortal(roleName) { state.activeRole = roleName; const el = document.getElementById('active-role-display'); if (el) el.innerText = roleName.toUpperCase(); document.querySelectorAll('.role-portal-page').forEach(el => el.style.display = 'none'); const targetPortal = document.getElementById('role-portal-' + roleName); if (targetPortal) targetPortal.style.display = 'block'; renderAllUI(); }
        ...
        function switchCustView(viewName) { document.querySelectorAll('.cust-view').forEach(el => el.style.display = 'none'); document.querySelectorAll('.navbar-custom .nav-link').forEach(el => el.classList.remove('active')); const target = document.getElementById('cust-view-' + viewName); if (target) target.style.display = 'block'; const navTarget = document.getElementById('cust-nav-' + viewName); if (navTarget) navTarget.classList.add('active'); if (viewName === 'checkout') prefillCheckoutForm(); if (viewName === 'poin') renderCustomerPointsPage(); }
        function updateStoreHoursStatus() { const now = new Date(); const hour = now.getHours(); const isOpenNow = hour >= 6 && hour < 16; state.isStoreOpen = isOpenNow; const alertEl = document.getElementById('closed-hours-alert'); const labelEl = document.getElementById('store-hours-label'); if (isOpenNow) { if (alertEl) alertEl.style.display = 'none'; if (labelEl) labelEl.innerHTML = '<span id="store-hours-dot" class="d-inline-block rounded-circle bg-success" style="width:8px;height:8px;"></span> BUKA (06.00 - 16.00)'; } else { if (alertEl) alertEl.style.display = 'block'; if (labelEl) labelEl.innerHTML = '<span id="store-hours-dot" class="d-inline-block rounded-circle bg-danger" style="width:8px;height:8px;"></span> TUTUP (16.00 - 06.00)'; } }
        function renderAllUI() { renderOutletDropdowns(); renderHomeProducts(); renderCatalogProducts(); renderCartUI(); renderCustomerHistory(); renderCustomerAuthArea(); renderCustomerPointsPage(); renderKasirPreOrders(); renderKasirLeftoverTable(); renderPosProductsGrid(); renderAdminProducts('adm-products-tbody', true); renderAdminProduction(); renderAdminInventory(); renderAdminDailyMenuGrid(); renderAdminOutletReports(); renderAdminPesananPerOutlet(); renderOwnerDashboard(); renderOwnerDailyMenuGrid(); renderOwnerProducts(); renderOwnerPreOrders(); renderOwnerProduction(); renderOwnerInventory(); renderOwnerOutletReports(); renderOwnerResetPasswordTable(); renderOwnerOutletsTable(); renderOwnerMembersTable(); renderOwnerRewardsTable(); renderOwnerProductPointsTable(); }
        function renderHomeProducts() { const grid = document.getElementById('home-products-grid'); if (!grid) return; grid.innerHTML = state.products.slice(0, 3).map(p => ` <div class="col-md-4"><div class="card-custom p-3 h-100 d-flex flex-column justify-content-between"><div>${productImageHtml(p, 140)}<span class="badge bg-warning text-dark fs-8 fw-bold mb-1">${p.age}</span><h6 class="fw-bold mb-1 text-dark">${p.name}</h6><div class="text-muted fs-8 mb-2">${p.ingredients}</div></div><div><div class="fw-bold text-brand-purple fs-6 mb-2">Rp ${p.price.toLocaleString('id-ID')}</div><button class="btn btn-brand-yellow btn-sm w-100 fw-bold text-dark" onclick="addToCart('${p.id}')"><i class="fa-solid fa-cart-plus me-1"></i> + Tambah Ke Keranjang</button></div></div></div>`).join(''); }
        function renderCatalogProducts() { const grid = document.getElementById('full-products-grid'); if (!grid) return; grid.innerHTML = state.products.map(p => ` <div class="col-md-4"><div class="card-custom p-3 h-100 d-flex flex-column justify-content-between"><div>${productImageHtml(p, 140)}<span class="badge bg-warning text-dark fs-8 fw-bold mb-1">${p.age}</span><h6 class="fw-bold mb-1 text-dark">${p.name}</h6><div class="text-muted fs-8 mb-2">${p.ingredients}</div></div><div><div class="fw-bold text-brand-purple fs-6 mb-2">Rp ${p.price.toLocaleString('id-ID')}</div><button class="btn btn-brand-yellow btn-sm w-100 fw-bold text-dark" onclick="addToCart('${p.id}')"><i class="fa-solid fa-cart-plus me-1"></i> + Tambah Ke Keranjang</button></div></div></div>`).join(''); }
        ...
        function showAddProductModal() { Swal.fire({ title: 'Tambah Varian MPASI Baru', html: `<div class="text-start mb-2"><label class="fw-bold fs-7 d-block mb-1">Foto Produk (opsional, maks 2MB)</label><img id="swal-pimg-preview" class="rounded-3 mb-2" style="width:100%; max-height:150px; object-fit:cover; display:none;"><input id="swal-pimage" type="file" accept="image/*" class="swal2-file"></div><input id="swal-pname" class="swal2-input" placeholder="Nama Varian MPASI"><input id="swal-pprice" class="swal2-input" type="number" placeholder="Harga / Cup (Rp)"><input id="swal-pstock" class="swal2-input" type="number" placeholder="Stok Ready Initial (Cup)"><input id="swal-pingredients" class="swal2-input" placeholder="Komposisi Bahan">`, focusConfirm: false, showCancelButton: true, confirmButtonText: 'Simpan Varian', confirmButtonColor: '#6A1B9A', didOpen: () => { bindImagePreview('swal-pimage', 'swal-pimg-preview'); }, preConfirm: async () => { const name = document.getElementById('swal-pname').value.trim(); const price = parseInt(document.getElementById('swal-pprice').value) || 0; const stock = parseInt(document.getElementById('swal-pstock').value) || 0; const ingredients = document.getElementById('swal-pingredients').value.trim(); if (!name || price <= 0) { Swal.showValidationMessage('Harap isi Nama dan Harga produk!'); return false; } const imageDataUrl = await readImageFileAsDataUrl(document.getElementById('swal-pimage')); return { name, price, stock, ingredients, image: imageDataUrl || '' }; } }).then((result) => { if (result.isConfirmed && result.value) { fetch('/api/products', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: JSON.stringify(result.value) }).then(res => res.json()).then(data => { if (data.success && data.product) { const newP = { id: data.product.id, name: data.product.name, price: data.product.price, stock: data.product.stock, initialStock: data.product.stock, category: data.product.category || 'Bubur', age: data.product.age_group || '6+ Bulan', ingredients: data.product.ingredients || 'Bahan segar alami', status: data.product.status || 'Aktif', image: data.product.image || '', customPoints: 0 }; state.products.push(newP); renderAllUI(); Swal.fire({ icon: 'success', title: 'Produk Ditambahkan', text: `${newP.name} berhasil disimpan!`, timer: 1200, showConfirmButton: false }); } }); } }); }
        ...
        function togglePreOrderTaken(orderId) { const item = state.preOrders.find(p => p.id == orderId); if (item) { item.isTaken = !item.isTaken; renderKasirPreOrders(); renderOwnerPreOrders(); } }
        function togglePaymentStatus(orderId) { const item = state.preOrders.find(p => p.id == orderId); if (item) { item.isPaid = !item.isPaid; renderKasirPreOrders(); renderOwnerPreOrders(); } }
        ...
        function renderPosProductsGrid() { const grid = document.getElementById('pos-products-grid'); if (!grid) return; grid.innerHTML = state.products.map(p => { const isOutOfStock = (p.stock || 0) <= 0; return ` <div class="col-6"><div class="card p-2 border text-center ${isOutOfStock ? 'bg-light opacity-50' : 'cursor-pointer bg-light'}" ${isOutOfStock ? '' : `onclick="addPosCart('${p.id}')"`}>${productThumbHtml(p, 56)}<div class="fw-bold fs-8 mt-1">${p.name}</div><div class="text-brand-purple fw-extrabold fs-8">Rp ${p.price.toLocaleString('id-ID')}</div><div class="mt-1">${isOutOfStock ? '<span class="badge bg-danger fs-8">STOK HABIS</span>' : `<span class="badge bg-primary fs-8"><i class="fa-solid fa-boxes-stacked me-1"></i> Stok: ${p.stock || 0} Cup</span>`}</div></div></div>`; }).join(''); }
        ...
        window.onload = function() { try { selectRolePortal(state.activeRole); renderAllUI(); updateStoreHoursStatus(); setInterval(updateStoreHoursStatus, 30000); } catch (err) { console.error("Render UI Error:", err); } finally { endLoading(); } };
    </script>
@endsection
